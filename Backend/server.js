...
//JWT Secret Key
const JWT_SECRET = '5b489a325035cb8a8380ac1dea230dd1267037a4d924b8efd490de4f18198860';
...
app.post('/master-login', async (req, res) => {
  ...
  try {
    ...
    if (user && user.password === password) {
      const token = jwt.sign({id: user._id, userType: 'admin'}, JWT_SECRET, {expiresIn: '1h'});
      res.status(200).json({ message: 'Login successful', token, userType: 'admin' });
    } else {
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (reporter && reporter.password === password) {
      const token = jwt.sign({id: reporter._id, userType: 'user'}, JWT_SECRET, {expiresIn: '1h'});
      res.status(200).json({
...
app.post('/admindashboard-new', upload.single('picture'), async (req, res) => {
  const { title, category, content, tags, pictureurl, isBreaking, reporterName, location, metaTitle, metaDescription } = req.body;
